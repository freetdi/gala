...
//				trace2("rev", y, i);
   ...
   template<template<class G> class CFG2>
	...
	~graph(){
		trace2("destroying graph", _num_edges, _v.size());
	}
...
//	trace2("resize v", i, nv);
	...
	for(auto v=GG->cbegin(); v!=GG->cend(); ++v){
...
struct copy_helper<Gsrc, Gtgt, true, true, true, true > {
	...
	//	auto psize=tgt._v.size();
		...
		for(typename Gsrc::iterator V=src.begin(); V!=src.end(); ++V){
